...
static void ksu_get_cmdline(char *full_comm, const char *comm, size_t buf_len)
{
	...
	if (in_atomic() || in_interrupt() || irqs_disabled() || !current->mm) {
	...
	/* ganti '\0' internal jadi spasi supaya readable */
	if (n >= buf_len)
		n = buf_len - 1;
			       ...
			       const char *content, size_t len)
{
	...
	u64 delta_ns = (u64)DEDUP_SECS * NSEC_PER_SEC;
...
static void sulog_schedule_task_work(void)
{
	...
	if (!cb) {
		pr_err("sulog: failed to alloc task_work cb\n");
		goto out_put;
	}
	...
	if (ret) {
		pr_err("sulog: task_work_add failed: %d\n", ret);
		kfree(cb);
	}
...
static void sulog_add_entry(char *log_buf, size_t len,
			    uid_t uid, u8 dedup_type)
{
	struct sulog_entry *entry;
	unsigned long flags;

	if (!sulog_enabled || !log_buf || !len)
		return;
...
void ksu_sulog_report_su_grant(uid_t uid,
			       const char *comm,
			       const char *method)
{
...
void ksu_sulog_report_su_attempt(uid_t uid,
				 const char *comm,
...
void ksu_sulog_report_permission_check(uid_t uid,
				       const char *comm,
				       bool allowed)
{
	char log_buf[SULOG_ENTRY_MAX_LEN];
					...
					uid_t target_uid)
{
	...
	snprintf(log_buf, sizeof(log_buf),
		 ...
		 manager_uid, target_uid,
		 full_comm, current->pid);

	sulog_add_entry(log_buf, strlen(log_buf),
			manager_uid, DEDUP_MANAGER_OP);
}

void ksu_sulog_report_syscall(uid_t uid,
			      const char *comm,
			      const char *syscall,
			      const char *args)
{
...
int ksu_sulog_init(void)
{
	if (ksu_register_feature_handler(&sulog_handler))
		pr_err("sulog: failed to register feature handler\n");
