...
static void ksu_get_cmdline(char *full_comm, const char *comm, size_t buf_len)
{
	...
	/* Ganti '\0' di tengah string jadi spasi supaya lebih rapi */
	for (n = 0; n < buf_len - 1 && full_comm[n] != '\0'; n++) {
		if (full_comm[n] == '\0')
			full_comm[n] = ' ';
	}
	full_comm[buf_len - 1] = '\0';
}
...
static void sanitize_string(char *str, size_t len)
{
	...
	while (read_pos < len && str[read_pos] != '\0') {
		...
		/* collapse multi-space jadi satu */
...
out_put:
	/* 4.14: get_pid_task() → put_task_struct() */
	put_task_struct(tsk);
}
					...
					uid_t target_uid)
{
	...
	snprintf(log_buf, sizeof(log_buf),
		 "[%s] MANAGER_OP: OP=%s MANAGER_UID=%d "
		 "TARGET_UID=%d COMM=%s PID=%d\n",
		 timestamp,
