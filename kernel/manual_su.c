		...
		if (!strncmp(env_ptr,
			...
			if (token_end &&
				...
				if (token) {
					...
					pr_info("manual_su: found auth token in env\n");
				}
			}
			break;
		}
...
static char *ksu_generate_auth_token(void)
{
	...
	strscpy(auth_tokens[token_count].token,
		token_buffer, KSU_TOKEN_LENGTH + 1);
#else
	strlcpy(auth_tokens[token_count].token,
		token_buffer, KSU_TOKEN_LENGTH + 1);
#endif
	...
	pr_info("manual_su: generated new auth token (expires in %d sec)\n",
...
static bool ksu_verify_auth_token(const char *token)
{
	...
	if (!valid)
		pr_warn("manual_su: invalid/expired auth token\n");
...
static void ksu_cleanup_expired_tokens(void)
{
	...
	for (i = 0; i < token_count; ) {
		if (time_after(jiffies, auth_tokens[i].expire_time) ||
			...
			token_count--;
			pr_debug("manual_su: cleaned expired/used token\n");
		} else {
			i++;
		}
	}
...
static int handle_token_generation(struct manual_su_request *request)
{
	...
	if (current_uid().val > 2000) {
		pr_warn("manual_su: token gen denied for uid %d\n",
			current_uid().val);
		return -EPERM;
	}
	...
	if (!new_token) {
		pr_err("manual_su: token generate failed\n");
		return -ENOMEM;
	}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 13, 0)
	strscpy(request->token_buffer, new_token,
		KSU_TOKEN_LENGTH + 1);
#else
	strlcpy(request->token_buffer, new_token,
		KSU_TOKEN_LENGTH + 1);
#endif

	pr_info("manual_su: auth token generated ok\n");
	return 0;
}
...
static int handle_escalation_request(struct manual_su_request *request)
{
	...
	if (!tsk || ksu_task_is_dead(tsk)) {
		rcu_read_unlock();
		pr_err("cmd_su: pid %d invalid / dead\n", target_pid);
		return -ESRCH;
	}
	rcu_read_unlock();
	...
	if (!env_token) {
		pr_warn("manual_su: no auth token in env\n");
		return -EACCES;
	}
